...
import { getAllBolts } from "../../services/boltService";
...
export default function Bolts() {
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="page_title">Bolts List</Typography>
        <Box>
          <TextField
            id="outlined-basic"
            label="Search bolts..."
      ...
      <Grid container spacing={3} mt={2}>
        {paginated.map((bolt) => (
          <Grid item size={{ xs: 12, sm: 12, md: 3, lg: 3 }} key={bolt.id}>
            <Card
              sx={{
                height: "100%",
                display: "flex",
            ...
            >
              <Box sx={{ position: "relative" }}>
                <CardMedia
                  component="img"
                  height="200"
                  image={bolt.images?.[0] || "/placeholder.svg"}
                  alt={bolt.name}
                  sx={{ objectFit: "cover", cursor: "pointer" }}
                  onClick={() => {
                    setImage(bolt?.images);
                    openLightbox(0);
                  }}
                />
              </Box>
              <CardContent
                sx={{ cursor: "pointer" }}
                // onClick={() => setProductDetailModal(bolt)}
              >
                <Typography variant="h6">{bolt.name}</Typography>
                <Stack
                  display="flex"
                  justifyContent="space-between"
                  flexDirection="row"
                ...
                <Typography color="text.secondary">
                  Color: {bolt.color || "N/A"}
                </Typography>
                <Typography fontWeight="bold">Price: £{bolt.price}</Typography>
                <Box
                  sx={{
                    display: "flex",
                    gap: 1,
                ...
                >
                  <Button
                    ...
                    onClick={() => handleAddToCart(bolt)}
                    disabled={bolt.quantity < 1}
                    sx={{
                      flex: 1,
                      bgcolor: "#EB3300",
